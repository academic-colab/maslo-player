...
-(BOOL) UnzipFileTo:(NSString*) path overWrite:(BOOL) overwrite unzippedDirectory:(NSMutableString*)dirName
{
	BOOL success = YES;
	int ret = unzGoToFirstFile( _unzFile );
	unsigned char		buffer[4096] = {0};
    BOOL seenFirstDirectory = NO;
	...
	do{
		...
		if( filename[fileInfo.size_filename-1]=='/' || filename[fileInfo.size_filename-1]=='\\')
			isDirectory = YES;
		free( filename );
        if (isDirectory && dirName != NULL && !seenFirstDirectory) {
            seenFirstDirectory = YES;
            [dirName appendString:strPath];
        }
